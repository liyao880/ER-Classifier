...
from utils import loaddata, loadmodel, free_params, frozen_params, clip_params, testattack, savefile
from setup_pgd import to_var, pred_batch_ot, adv_train_ot, LinfPGDAttackOT

        
def train_er_classifier(train_loader, test_loader, encoder, discriminator, classifier,
                 use_cuda, n_z, sigma, num_epoch, lr, LAMBDA, LAMBDA0, LAMBDA1, file_name, 
                 epsilon, k, a, delay, print_every, dataset, attack_range):
...
def main(args):
    ...
    classifier, encoder = train_er_classifier(train_loader, test_loader, encoder, discriminator, classifier,
                 ...
                 print_every=args['print_every'], dataset=args['dataset'], attack_range=args['range'])
    
    test(test_loader, classifier, encoder=encoder, use_cuda=True)
    print('==> Testing the model against PGD attack..')
    testattack(classifier, encoder, test_loader, epsilon=args['epsilon'], 
               k=args['k'], a=args['a'], dataset=args['range'], use_cuda=True)

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='PyTorch Embedding Regularized Classifier')
    parser.add_argument('-d', '--dataset', choices=["mnist", "cifar10", "stl10"], default="mnist")    
    parser.add_argument('-epochs', type=int, default=30, help='number of epochs to train (default: 30)')
    parser.add_argument('-file_name', default="/n3_")
    parser.add_argument('-init', default=None)
    parser.add_argument('-root', default="/home/mli/yaoli")
    ...
    parser.add_argument('-enc_init', default="/encoder_f_init")
    parser.add_argument('-shuffle', default="True")    
    args = vars(parser.parse_args())
    if args['dataset'] == 'mnist' or args['dataset'] == 'stl10':
        args['range'] = 'mnist'
    else:
        args['range'] = 'cifar'
